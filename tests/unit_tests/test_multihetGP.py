...
def test_fit():
    GPlist = [
        hetGP().mle(X=X,Z=Y[:,i],covtype=COVTYPE,maxit=MAXIT,
                    settings=SETTINGS,noiseControl=NOISECONTROL)
        for i in range(Y.shape[1])
    ]
    multiGP = emulator(x=X,theta=np.array([0,1]).reshape(2,1),f=Y,
                       method='multihetGP',args={'covtype':COVTYPE,'maxit':MAXIT,'settings':SETTINGS,'noiseControl':NOISECONTROL})
    #multiGP.fit()
...
def test_predict():
    ...
    multiGP = emulator(x=X,theta=np.array([0,1]).reshape(2,1),f=Y,
                       method='multihetGP',args={'covtype':COVTYPE,'maxit':MAXIT,'settings':SETTINGS})
    multiGP.fit()
    preds = multiGP.predict(x=Xp,thetaprime=Xp)
    GPlist = [
        hetGP().mle(X=X,Z=Y[:,i],covtype=COVTYPE,
                    maxit=MAXIT,settings=SETTINGS
