...
def test_fit():
    ...
    multiGP = emulator(
        ...
        f=Y,
        method="multihomGP",
        args={"covtype": COVTYPE, "maxit": 50, "settings": SETTINGS},
    )
    multiGP.fit()
    GPlist = [
        homGP().mle(X=X, Z=Y[:, i], covtype=COVTYPE, maxit=50, settings=SETTINGS)
...
def test_predict():
    ...
    GPlist = [
        homGP()
        .mle(X=X, Z=Y[:, i], covtype=COVTYPE, maxit=50, settings=SETTINGS)
