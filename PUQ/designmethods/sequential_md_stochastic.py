...
class sequential_design:
    ...
    def build_design(self, t0, f0, af, args={}):
        ...
        rand_stream = np.random.default_rng(
            seed * (b + 1)
        ...
        is_explore, is_exploit = des_settings.get("is_explore"), des_settings.get(
            "is_exploit"
        )

        for iteration in range(max_iter):

            emu, TV = self.newiteration(
                t_c, f_c, self.x, pc_settings, test_data, self.Sigma, self.y
            )

            theta, iter_exploit, iter_explore = self.onebatch(
                b,
                b_new,
                r_new,
                emu,
