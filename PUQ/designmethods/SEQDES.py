...
def gen_f(H, persis_info, gen_specs, libE_info):
    ...
    prior_func, prior_func_x, prior_func_t = (
        prior_func_all["prior"],
        prior_func_all["priorx"],
        prior_func_all["priort"],
    )

    # Simulation info
    synth_info = gen_specs["user"]["synth_cls"]
    obsvar, data, theta_limits, dim, x = (
        synth_info.obsvar,
        synth_info.real_data,
        synth_info.thetalimits,
        synth_info.d,
        synth_info.x,
    )
    ...
    if test_data is not None:
        thetatest, th_mesh, x_mesh, ptest, ftest, priortest, ytest = (
            test_data["theta"],
            test_data["th"],
            test_data["xmesh"],
            test_data["p"],
    ...
    while tag not in [STOP_TAG, PERSIS_STOP]:
        ...
        if update_model:
            ...
            if is_thetamle:
                ...
                theta_mle = find_mle(
                    emu, x, x_emu, true_fevals, obsvar, dx, dt, theta_limits, False
                )
        ...
        if first_iter:
            ...
            if select_condition(
                ...
                if AL == None:
                    new_theta = theta_torun[
                        (generated_no - n_init) : (generated_no - n_init + mini_batch),
                        :,
                    ]
                else:
