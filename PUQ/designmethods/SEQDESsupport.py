...
def fit_emulator1d(x_emu, theta, fevals):
    ...
    emu = emulator(x_emu, theta, fevals, method="PCGPexp")

    return emu


def compute_diff(x, parameter, nx, emu, x_emu, obs, is_bias):
    # obs : 1 x n_x
    # emumean : 1 x n_x
    # bias : n_x x 1
    dt = len(parameter)
    dx = x.shape[1]
    xp = [
        np.concatenate([xc.reshape(1, dx), parameter.reshape(1, dt)], axis=1)
        for xc in x
    ]
...
def obj_mle(parameter, args):
    emu, x, x_emu, obs, obsvar, is_bias = (
        args[0],
        args[1],
        args[2],
        args[3],
...
def find_mle(emu, x, x_emu, obs, obsvar, dx, dt, theta_limits, is_bias):
...
def collect_data(
    ...
    dx = xmesh.shape[1]
    xtrue_test = [
        np.concatenate([xc.reshape(1, dx), theta_mle], axis=1) for xc in xmesh
    ]
...
def bias_predict(emu, theta_mle, x_emu, x, obs, unknowncov=False):
