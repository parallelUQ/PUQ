    ...
    def __init__(
        ...
        if alloc_settings.get("theta") is None:
            if alloc_settings.get("gen") is False:
                ...
                self.theta = np.concatenate(
                    (self.emu._info["emulist"][0]["X0"], self.theta), axis=0
                )
        else:
        ...
        if alloc_settings.get("a0") is None:
            if alloc_settings.get("gen") is False:
                ...
                self.a0 = np.concatenate(
                    (self.emu["emulist"]._info[0]["mult"], np.repeat(0, 100))
                )
        else:
            ...
            for j in range(0, q):
                ...
                if emuinfo["is_homGP"]:
                    Mb[i, :, j, j] = (
                        -emuinfo["nu_hat"]
                        * emuinfo["g"]
                        * np.einsum("ji,jk,ki->i", K_s, A, K_s)
                    )
                else:
                    Mb[i, :, j, j] = -(pred_nugs._info["nugs_o"][j, i]) * np.einsum(
                        "ji,jk,ki->i", K_s, A, K_s
                    )
    ...
    def compute_ivar_weights(self):
        ...
        for i in range(0, n):
            ...
            for j in range(0, q):
                ...
                A = Ki @ J @ Ki
                if emuinfo["is_homGP"]:
                    Mb[i, :, j, j] = -(emuinfo["g"] * emuinfo["nu_hat"]) * np.einsum(
                        "ji,jk,ki->i", K_s, A, K_s
                    )
                else:
                    # Mb[i, :, j, j] = -(pred_nugs._info['nugs_o'][j, i])*np.einsum('ji,jk,ki->i', K_s, A, K_s)
                    Mb[i, :, j, j] = -(pred_nugs._info["nugs"][j, i]) * np.einsum(
                        "ji,jk,ki->i", K_s, A, K_s
                    )
