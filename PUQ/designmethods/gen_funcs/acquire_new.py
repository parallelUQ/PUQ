...
class acquire:
    ...
    def seivar(self):
        ...
        for i in range(self.bnew):
            ...
            if self.impute_str == "update":
                update(emu._info, x=x, X0new=ct, mult=self.rep)
                fE, tE = impute(ct=ct, x=x, fE=fE, tE=tE, reps=self.rep, emu=emu, rnd_str=self.rand_stream)
            else:
                if self.impute_str == "KB":
                    fE, tE = impute(ct=ct, x=x, fE=fE, tE=tE, reps=self.rep, emu=emu, rnd_str=self.rand_stream)
                elif self.impute_str == "CL":
                    fE, tE = impute_CL(ct=ct, x=x, fE=fE, tE=tE, reps=self.rep, liar=liar)
                
                emu = build_emulator(x=x, theta=tE, f=fE, pcset=self.pc_settings)
        ...
        if self.skip:
            ...
            emu = build_emulator(x=x, theta=tE, f=fE, pcset=self.pc_settings)
            ivar = compute_ivar(emu=emu, ttest=self.tmesh, x=x, obs=obs, obsvar=obsvar)
        self.tnew = tnew
        self.ivar = ivar
        return 
