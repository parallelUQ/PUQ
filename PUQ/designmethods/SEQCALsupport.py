...
def select_condition(complete, prev_complete, n_theta=2, n_initial=10):
    ...
    if (np.sum(complete) - np.sum(prev_complete) < n_theta) or (
        np.sum(complete) < n_initial
    ):
        nflag = False
    else:
        nflag = True
...
def pad_arrays(
    n_x, thetanew, theta, fevals, pending, prev_pending, complete, prev_complete
):
...
def update_arrays(
    n_x, fevals, pending, complete, calc_in, obs_offset, theta_offset, list_id
):
...
def fit_emulator(x, theta, fevals, thetalimits):
    ...
    emu = emulator(x, theta_c, fevals_c, method="PCGP")
