...
def plot_workers(PM, joblist, acqlist):
    ...
    for jid, job in enumerate(joblist):
        if job.end <= PM.complete_time:
            ax.hlines(y = job.workerid+1, xmin = job.start, xmax = job.end, color='b', linewidth=1)
            ax.vlines(x = job.start, ymin = job.workerid+1-0.25, ymax = job.workerid+1+0.25, color='b', linestyles='dashed' , linewidth=1)
            ax.vlines(x = job.end, ymin = job.workerid+1-0.25, ymax = job.workerid+1+0.25, color='b', linestyles='dashed' , linewidth=1)
...
def plot_acqtime(axes, n, acclevel, rlist, labellist, logscale=False, fontsize=18, ind=False, n0=0):
    clist = ['blue', 'red', 'green', 'magenta', 'orange', 'dimgrey', 'lime', 'dimgrey', 'lime']

    for accid, res in enumerate(rlist):
        if ind:
            axes.plot(np.arange(1, n+1), res.gentime, label=labellist[accid], color=clist[accid])
        else:
