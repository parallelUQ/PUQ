...
def predict(predinfo, fitinfo, x, theta, **kwargs):
    ...
    for k in range(0, len(infos)):
        if infos[k]["hypind"] == k:
            ...
            rsave_3[k] = __covmat(theta, theta, infos[k]["hypcov"])
        ...
        predcovs[:, :, k] = infos[k]["sig2"] * (
            r3 - rVh @ rVh.T
        )  # np.abs(1 - np.sum(rVh ** 2, 1))
...
def postphimat(
    fitinfo, n_x, theta, obs, obsvar, theta_cand, covmat_ref, rVh_1_3D, pred_mean
):
    ...
    for k in range(0, len(infos)):
        ...
        if infos[k]["hypind"] == k:
            rsave_2[k] = __covmat(theta, theta_cand, infos[k]["hypcov"])

            rsave_4[k] = __covmat(theta_cand, fitinfo["theta"], infos[k]["hypcov"])
    ...
    var_cand = (
        (
            fitinfo["standardpcinfo"]["extravar"][:]
            + predvars_cand @ (pctscale[:, :] ** 2).T
        )
    ).T
...
def temp_postphimat(fitinfo, n_x, theta, obs, obsvar):
    ...
    for k in range(0, len(infos)):
        if infos[k]["hypind"] == k:
            # covariance matrix between new theta and thetas from fit.
            rsave_1[k] = __covmat(theta, fitinfo["theta"], infos[k]["hypcov"])
...
def postpred(fitinfo, x, theta, obs, obsvar):
    ...
    predinfo["mean"] = (
        (predmean_ref @ pctscale.T) + fitinfo["standardpcinfo"]["offset"]
    ).T
