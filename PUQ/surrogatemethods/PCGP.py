...
def acquisition(fitinfo, x, theta1, theta2, **kwargs):
    ...
    for k in range(0, len(infos)):
        if infos[k]['hypind'] == k:
            # covariance matrix between new theta and thetas from fit.
            rsave_1[k] = __covmat(theta1,
                                  fitinfo['theta'],
                                  infos[k]['hypcov'])
            rsave_2[k] = __covmat(theta2,
                                  fitinfo['theta'],
                                  infos[k]['hypcov'])
            rsave_3[k] = __covmat(theta1,
                                  theta2,
                                  infos[k]['hypcov'])
        ...
        predcov[:, :, k] = infos[k]['sig2'] * (r_3.reshape((theta1.shape[0], theta2.shape[0])) - rVh_1 @ rVh_2.T)
