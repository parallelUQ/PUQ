...
def predict(predinfo, fitinfo, x, theta, **kwargs):
    ...
    if (
        (kwargs is not None)
        and ("return_grad" in kwargs.keys())
        and (kwargs["return_grad"] is True)
    ):
        return_grad = True
    return_covx = True
    if (
        (kwargs is not None)
        and ("return_covx" in kwargs.keys())
        and (kwargs["return_covx"] is False)
    ):
        return_covx = False
    ...
    try:
        if (
            x is None
            or np.all(np.equal(x, fitinfo["x"]))
            or np.allclose(x, fitinfo["x"])
        ):
    ...
    for k in range(0, len(infos)):
        if infos[k]["hypind"] == k:
            ...
            if return_grad:
                rsave[k], drsave[k] = __covmat(
                    theta, fitinfo["theta"], infos[k]["hypcov"], return_gradx1=True
                )
            else:
    ...
    predinfo["mean"][xnewind, :] = (
        (predvecs @ pctscale[xind, :].T) + fitinfo["standardpcinfo"]["offset"][xind]
    ).T
    predinfo["var"][xnewind, :] = (
        (
            fitinfo["standardpcinfo"]["extravar"][xind]
            + predvars @ (pctscale[xind, :] ** 2).T
        )
    ).T
    ...
    if return_grad:
        ...
        predinfo["mean_gradtheta"][xnewind, :, :] = (
            (predvecs_gradtheta.transpose(0, 2, 1) @ pctscale[xind, :].T)
        ).transpose((2, 0, 1))
        ...
        if return_covx:
            ...
            dsqrtpredvars = 0.5 * (
                predvars_gradtheta.transpose(2, 0, 1) / np.sqrt(predvars)
            ).transpose(1, 2, 0)
            ...
            if np.allclose(xnewind, xind):
                ...
                predinfo["covxhalf_gradtheta"] = np.full(
                    (x.shape[0], theta.shape[0], CH.shape[1], theta.shape[1]), np.nan
                )
...
def acquisition(fitinfo, x, theta1, theta2, **kwargs):

    infos = fitinfo["emulist"]
    predcov = np.zeros((theta1.shape[0], theta2.shape[0], len(infos)))
    tau = np.zeros((theta1.shape[0], theta2.shape[0], len(infos)))
    predvars_2 = np.zeros((theta2.shape[0], len(infos)))
    try:
        if (
            x is None
            or np.all(np.equal(x, fitinfo["x"]))
    ...
    for k in range(0, len(infos)):
        if infos[k]["hypind"] == k:
            # covariance matrix between new theta and thetas from fit.
            rsave_1[k] = __covmat(theta1, fitinfo["theta"], infos[k]["hypcov"])
            rsave_2[k] = __covmat(theta2, fitinfo["theta"], infos[k]["hypcov"])
            rsave_3[k] = __covmat(theta1, theta2, infos[k]["hypcov"])
        ...
        if rVh_2.ndim < 1.5:
            rVh_2 = rVh_2.reshape((1, -1))

        # print('rVh1:', rVh_1.shape)
        # print('rVh2:', rVh_2.shape)

        # predvars_1[:, k] = infos[k]['sig2'] * np.abs(1 - np.sum(rVh_1 ** 2, 1))

        # predcov[:, :, k] = infos[k]['sig2'] * (r_3.reshape((theta1.shape[0], 1)) - rVh_1 @ rVh_2.T)
        predcov[:, :, k] = infos[k]["sig2"] * (
            r_3.reshape((theta1.shape[0], theta2.shape[0])) - rVh_1 @ rVh_2.T
        )
        ...
        tau[:, :, k] = (predcov[:, :, k] ** 2) / predvars_2[:, k]
        # print('tau:', tau.shape)
    pctscale = (fitinfo["pcti"].T * fitinfo["standardpcinfo"]["scale"]).T
    ...
    phi_mat_torch = torch.matmul(Btau_torch, Btau_torchT)
    ...
    # phi_matrix2[i, :, :, j] =  pctscale@np.diag(tau[i, j, :])@pctscale.T
    # phi_matrix[i, :, :, j] = Btau[i, j, :, :]@Btau[i, j, :, :].T

    # print(phi_matrix[1, :, :, 0])
    # print(phi_matrix2[1, :, :, 0])
    return phi_mat_np  # phi_matrix
...
def predictlpdf(predinfo, f, return_grad=False, addvar=0, **kwargs):
    ...
    for c in range(0, predinfo["predvars"].shape[0]):
        ...
        if return_grad:
            Si = (v * (1 / w)) @ v.T
            grt = (
                predinfo["predvars_gradtheta"][c, :, :].T / predinfo["predvars"][c, :]
            ).T
...
def __fitGP1d(
    ...
    subinfo["hypregmean"] = np.append(
        0 + 0.5 * np.log(theta.shape[1]) + np.log(np.std(theta, 0)),
        (0, hypvarconstmean, hyp1),
    )
    subinfo["hypregLB"] = np.append(
        -4 + 0.5 * np.log(theta.shape[1]) + np.log(np.std(theta, 0)),
        (-12, hypvarconstLB, hyp2),
    )
...
def __negloglikgrad(hyp, info):
    ...
    for k in range(0, dR.shape[2]):
        dsig2hat = -np.sum(
            (Vh @ np.multiply.outer(fcenter, fcenter) @ Vh.T) * dR[:, :, k]
        ) / (n + sig2ofconst)
