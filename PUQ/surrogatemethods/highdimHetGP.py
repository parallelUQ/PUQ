...
def fit(
    ...
    **kwargs
    ...
    for i in range(0, numGPs):
        ...
        emulist[i] = model.copy()#emu._info
...
def predict(predinfo, fitinfo, x, theta, thetaprime, **kwargs):
    ...
    for i in range(0, numGPs):
        ...
        # )
        pr = info.predict(x=theta, xprime=thetaprime)
 
        mean_pc[i, :] = pr["mean"] # predinfo_hetGP["mean"]
        var_pc[i, :] = pr["sd2"] # predinfo_hetGP["var"]
        nugs_pc[i, :] = pr["nugs"] # predinfo_hetGP["nugs"]

        if thetaprime is not None:
            covmat_pc[i, :, :] = pr["cov"] # predinfo_hetGP["covmat"]
