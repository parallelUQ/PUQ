...
class emulator(object):
                ...
                if self.__theta.shape[1] == theta.shape[1]:
                    if thetachoices is None:
                        if theta.shape[0] > 30 * size:
                            thetachoices = theta[
                                np.random.choice(
                                    theta.shape[0], 30 * size, replace=False
                                ),
                                :,
                            ]
                        else:
                    ...
                    raise ValueError(
                        "theta has the wrong shape, it does not "
                        "match emu._emulator__theta."
                    )

        try:
            supptheta, suppinfo = self.method.supplementtheta(
                self._info,
                copy.copy(size),
                copy.copy(theta),
        ...
        if supptheta is not None:
            if removereps:
                ...
                if nctheta.shape[0] < 0.5:
                    supptheta = None
                    raise ValueError(
                        "supptheta is a complete replication of " "self.__theta."
                    )
                else:
            ...
            if (self.__supptheta is not None) and (not overwrite):
                raise ValueError(
                    "Either evaluate self.__supptheta or " "select overwrite = True."
                )
            else:
    ...
    def update(self, x=None, theta=None, f=None, args=None, options=None):
        ...
        if (theta is not None) and (x is not None):
            ...
            raise ValueError(
                "Adding new theta and x at once is currently not "
            ...
            if (theta is None) and (x is None):
                    ...
                    if self.__supptheta is not None:
                        if f.shape[1] == self.__supptheta.shape[0]:
                            if self.__options["thetareps"]:
                                ...
                                self.__theta = np.vstack(
                                    (self.__theta, self.__supptheta)
                                )
                                self.__f = np.hstack((self.__f, f))
                                self.__supptheta = None
                            else:
                                # identify matches
                                nc, c, r = _matrixmatching(
                                    self.__theta, self.__supptheta
                                )
                                ...
                                if nc.shape[0] > 0.5:
                                    ...
                                    self.__f = np.hstack((self.__f, f))
                                    self.__theta = np.vstack((self.__theta, supptheta))
                                    self.__supptheta = None
                        else:
                            raise ValueError(
                                "Could not resolve absense of "
                        ...
                        raise ValueError(
                            "Could not resolve absense of theta, "
                            "please provide theta"
                        )
                else:
                    # no of rows of f (and x) is not the same
                    raise ValueError("Could not resolve absense of x. Provide " "x.")
            elif theta is not None:
                if (
                    (f.shape[0] == self.__f.shape[0])
                    and (f.shape[1] == theta.shape[0])
                    and (theta.shape[1] == self.__theta.shape[1])
                ):
                    if self.__options["thetareps"]:
                    ...
                    raise ValueError(
                        "Check the dimensions of theta and f. "
                        "Possible solutions: "
                ...
                if (
                    (f.shape[1] == self.__f.shape[1])
                    and (f.shape[0] == x.shape[0])
                    and (x.shape[1] == self.__x.shape[1])
                ):
                    if options["xreps"]:
                    ...
                    raise ValueError(
                        "Check the dimensions of x and f. "
                        "Possible solutions: "
            ...
            if x.shape[0] != self.__f.shape[0]:
                raise ValueError(
                    "Number of rows of x is changed but new f is " "not provided."
                )
            else:
                self.__x = x
            ...
            if theta.shape[0] != self.__f.shape[1]:
                raise ValueError(
                    "Number of rows of theta is changed but new " "f is not provided."
                )
            else:
                self.__theta = theta

        if "update" in dir(self.method):
            x, theta, f = self.__preprocess()
            self.method.update(self._info, x, theta, f, **self._args)
        else:
    ...
    def remove(self, x=None, theta=None, cal=None, options=None):
        ...
        if cal is not None:
            totalseen = np.where(
                np.mean(np.logical_not(np.isfinite(self.__f)), 0)
                < self.__options["thetarmnan"]
            )[0]
...
class prediction(object):
    ...
    def rnd(self, s=100, args=None):
        ...
        if (self.emu._emulator__ptf is None) and (
            ...
            if args is None:
                args = self.emu._args
            return copy.deepcopy(self.emu.method.ldf(self._info, **args))
        else:
            raise ValueError(self.__methodnotfoundstr(pfstr, opstr))
    ...
    def lpdf(self, f=None, args=None):
        ...
        if (self.emu._emulator__ptf is None) and (
            ...
            if args is None:
                args = self.emu._args
            return copy.deepcopy(self.emu.method.predictlpdf(self._info, f, **args))
        else:
            raise ValueError(self.__methodnotfoundstr(pfstr, opstr))
    ...
    def lpdf_gradtheta(self, f=None, args=None):
        ...
        if (self.emu._emulator__ptf is None) and (
            ...
            return copy.deepcopy(
                self.emu.method.predictlpdf_gradtheta(self._info, f, **args)
            )
        else:
            raise ValueError(self.__methodnotfoundstr(pfstr, opstr))
...
def _matrixmatching(mat1, mat2):
    ...
    if (mat1.shape[0] > (10 ** (4))) or (mat2.shape[0] > (10 ** (4))):
        raise ValueError(
            "too many matchings attempted." "Don" "t make the method work so hard!"
        )
    if mat1.ndim != mat2.ndim:
        raise ValueError("Somehow sent non-matching information to" " _matrixmatching")
    if mat1.ndim == 1:
        matchingmatrix = np.isclose(mat1[:, None].astype("float"), mat2.astype("float"))
    else:
        matchingmatrix = np.isclose(
            mat1[:, 0][:, None].astype("float"), mat2[:, 0].astype("float")
        )
        for k in range(1, mat2.shape[1]):
            try:
                matchingmatrix *= np.isclose(
                    mat1[:, k][:, None].astype("float"), mat2[:, k].astype("float")
                )
            except Exception:
