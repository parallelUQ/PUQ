...
def twoD(designobj, Xpl, Ypl, p_test, nmesh):
    ...
    for label, x_count, y_count in zip(reps0, theta0[:, 0], theta0[:, 1]):
        if label == 2:
            plt.annotate(label, xy=(x_count, y_count), xytext=(0, 0), textcoords='offset points', fontsize=12, color='cyan', weight='bold')            
        else:
            plt.annotate(label, xy=(x_count, y_count), xytext=(0, 0), textcoords='offset points', fontsize=12, color='blue', weight='bold')
...
def test_data_gen_1d(cls_func, nmesh):
    ...
    # Create test data
    theta_test = np.linspace(cls_func.thetalimits[0][0], cls_func.thetalimits[0][1], nmesh)[:, None]
...
def oned(seqobject, x, obsvar, real_data, theta_test, f_test, p_test, cls_func, ninit):
    ...
    #     linestyle="dotted",
    # )
    ax.scatter(theta.flatten(), f.flatten(), s=60,facecolors='none', edgecolors='green')
    ...
    for tid in range(0, len(theta_test)):
        ...
        phatvar[tid] = (1/(2*np.sqrt(np.pi)*np.sqrt(obsvar)))*pvar1[tid] - phat[tid]**2
    ...
    for label, x_count, pval in zip(reps0, theta0, p_alloc):
        plt.annotate(label, xy=(x_count, pval), xytext=(0, 0), textcoords='offset points', color='blue', size=20)
    ax.set_xlabel(r"$\theta$", fontsize=ft)
    ax.set_ylabel(r"$p(y|\theta)$", fontsize=ft)
    ax.tick_params(axis="both", labelsize=ft)
    plt.show()
    ...
    fig, ax = plt.subplots()
    ax.plot(theta_test.flatten(), emupred._info['nugs'].flatten(), color='blue', linestyle="dashed", linewidth=2.5)
    ...
    p1 = host.scatter(theta0.flatten(), reps0.flatten(), color="black", marker='*', s=55, label=r"$a_i$")
    p2, = par1.plot(theta_test.flatten(), emupred._info['nugs'].flatten(), "b", linestyle="dashed", label="Variance", linewidth=2.5)
    p3, = par2.plot(theta_test, phat, "r", linestyle="dotted", label="Likelihood", linewidth=2.5)
    
    host.set_xlim(-0.1, 1.1)
    host.set_ylim(0.9*np.min(reps0.flatten()), 1.1*np.max(reps0.flatten()))
    par1.set_ylim(0.9*np.min(emupred._info['nugs'].flatten()), 1.1*np.max(emupred._info['nugs'].flatten()))
    ...
    host.legend(lines, [l.get_label() for l in lines], loc='center', bbox_to_anchor=(0.5, -0.3), ncol=3, prop={"size": ft})
    ...
    if cls_func.data_name in ['unimodal', 'branin']:
        ...
        for i in range(nmesh+1):
            for j in range(nmesh+1):
                ...
                P[i, j] = cls_func.noise(np.array([X[i, j], Y[i, j]])[None, :]).flatten()
...
def Figure2(desobject, theta_test, nugs, phat, method):
    ...
    p1 = host.scatter(theta0.flatten(), reps0.flatten(), color="black", marker='*', s=55, label=r"$a_i$")
    p2, = par1.plot(theta_test.flatten(), nugs, "b", linestyle="dashed", label="Variance", linewidth=2.5)
    p3, = par2.plot(theta_test, phat, "r", linestyle="dotted", label="Likelihood", linewidth=2.5)
