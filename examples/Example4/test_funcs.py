...
class unimodal:
    ...
    def realdata(self, seed):
        ...
        M = np.array(
            self.function(self.theta_true[0], self.theta_true[1]), dtype="float64"
        )
        if seed is None:
            self.real_data = M
        else:
            persis_info = {"rand_stream": np.random.default_rng(seed)}
            R = persis_info["rand_stream"].normal(0, np.sqrt(self.obsvar[0]), size=1)
            self.real_data = M + R
...
class bimodal:
    ...
    def sim_f(self, thetas, persis_info):
        ...
        f = self.function(thetas[0], thetas[1])
        V = self.noise(np.array([thetas[0], thetas[1]])[None, :])
        R = persis_info["rand_stream"].multivariate_normal(
            np.array([0, 0]), np.diag(V.flatten()), 1
        )
        f += R.flatten()
        return f
    ...
    def realdata(self, seed):
        ...
        if seed is None:
            ...
            persis_info = {"rand_stream": np.random.default_rng(seed)}
            R = persis_info["rand_stream"].multivariate_normal(
                mean=[0, 0], cov=self.obsvar, size=1
            )
            self.real_data = M + R
...
class sinf:
    ...
    def sim_f(self, thetas, persis_info):
        ...
        noise = persis_info["rand_stream"].normal(0, np.sqrt(var_noise[0]), 1)
        f += noise
        return f
    ...
    def realdata(self, seed):
        ...
        if seed is None:
            ...
            persis_info = {"rand_stream": np.random.default_rng(seed)}
            noise = persis_info["rand_stream"].normal(
                0, np.sqrt(self.obsvar[0]), size=1
            )
...
class branin:
    ...
    def noise(self, theta):
        return np.repeat(1, self.d * theta.shape[0]).reshape(self.d, theta.shape[0])
