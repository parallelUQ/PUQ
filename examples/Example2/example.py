...
if __name__ == "__main__":
    ...
    for d_id in range(len(cex.x)):
        ax[0].scatter(
            cex.x[d_id, 0], cex.real_data[0, d_id], color="black", s=50
        )
    ft = 16
    ...
    ax[1].scatter(des_obj.zs[n0:, 0], des_obj.zs[n0:, 1], marker="+", c="red", s=200, linewidth=3)
    ax[1].hlines(cex.true_theta, 0, 1, linestyles="dotted", linewidth=3, colors="orange",) 
    for xitem in x_obs:
        ax[1].vlines(xitem, 0, 1, linestyles="dotted", colors="orange", linewidth=3, zorder=1)
