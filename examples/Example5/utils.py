...
def heatmap(Xpl, Ypl, noise_grid, f_grid, p_grid, t_sample=None):
    ...
    cs = ax[0].contourf(
        Xpl, Ypl, noise_grid.reshape(50, 50), cmap=yellow_cmap, alpha=0.75
    )
...
def toy_example(cls_data, Xpl, Ypl, fg, ng, persis_info):
    ...
    fig, axs = plt.subplots(
        1, 3, figsize=(14, 3.5), constrained_layout=False
    )  # Disable constrained_layout
    fig.subplots_adjust(
        wspace=0.4, left=0.1, right=0.9
    ...
    )  # Adjust pad for the colorbar
    cs1 = axs[0].contourf(Xpl, Ypl, fg.reshape(50, 50), cmap=yellow_cmap, alpha=0.75)
    fig.colorbar(cs1, cax=cax1)

    xt_joint = np.column_stack(
        (cls_data.x, np.repeat(cls_data.theta_true, len(cls_data.x)))
    )
    ...
    for k in range(10):
        axs[2].scatter(
            ...
            np.array(
                [
                    cls_data.sim_f(np.array([x, cls_data.theta_true[0]]), persis_info)
                    for x in xs
                ]
            ),
            facecolors="none",
...
def fig6(desobj, theta0, rep0, Xpl, Ypl, pg, ng, ax, figset={}, fig={}, exp=True):
    ...
    xtick, ytick, cbarf = (
        figset.get("xtick", True),
        figset.get("ytick", True),
        figset.get("cbar", True),
    )
...
def save_output(desing_obj, name, al_func, seedno, path=None, label=None):
    ...
    if path is None:
        ...
        design_path = (
            path + name + "_" + al_func + "_seed_" + str(seedno) + "_" + label + ".pkl"
        )
