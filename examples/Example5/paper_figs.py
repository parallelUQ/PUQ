...
def plot_SIR_experiment_figure(
    ...
    fig, ax = plt.subplots(1, 4, figsize=figsize, gridspec_kw={"wspace": 0.25})
    ...
    if usual_boxplot:
        ...
        boxplot_alternative(
            path, ms, ls, "SIR", ax=ax[1], figset=figset
        )  # {"legend_right":False})
        box_data = [
            (df3, "MADy", ax[2], r"$MAD^y$"),
            (df3, "MADn", ax[3], r"$MAD^n$"),
...
def plot_sir_figure(
    ...
    for r in np.arange(repid[0], repid[1]):
        # Plotting
        fig, ax = plt.subplots(
            1, len(methods), figsize=(11, 3), constrained_layout=True
        )
...
def plot_pritam(path, repid=1, label="target", title=None):
    ...
    for mid, m in enumerate(ms):
        ...
        ax[0, mid].hist(
            desobj.zs[n0:, 2], bins=30, edgecolor="black", alpha=0.75, color="blue"
        )
        ...
        for xpt in xr:
            ax[1, mid].scatter(
                xpt[0], xpt[1], marker="x", color="black", s=100, zorder=3
            )

        # Contour of noise
        cs = ax[1, mid].contourf(
            X1, X2, ng.reshape(nmesh, nmesh), cmap=yellow_cmap, alpha=0.75, zorder=1
        )
    ...
    for m in methods:
        for hor in horizons:
            for i in np.arange(1, 31):
                ...
                unique_init, counts_init = np.unique(
                    initial_des, axis=0, return_counts=True
                )
