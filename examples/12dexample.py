...
from plots_design import create_test_12x, add_result, samplingdata, observe_results
from ptest_funcs import twelved
...
if __name__ == "__main__":
    for s in np.arange(args.seedmin, args.seedmax):
        ...
        prior_t      = prior_dist(dist='uniform')(a=cls_data.thetalimits[6:12, 0], b=cls_data.thetalimits[6:12, 1])
    
        priors = {'prior': prior_xt, 'priorx': prior_x, 'priort': prior_t}
    
        xt_test, ftest, ptest, thetamesh, xmesh = create_test_12x(cls_data)
        ...
        plt.scatter(xt_eivarx[ninit:, 0], xt_eivarx[ninit:, 1])
        plt.show()
        ...
        import matplotlib.pyplot as plt
        plt.scatter(xt_eivar[ninit:, 0], xt_eivar[ninit:, 1])
        plt.show()
