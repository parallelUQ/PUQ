...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 28 17:08:18 2022
...
xpl = np.linspace(cls_ackley.thetalimits[0][0], cls_ackley.thetalimits[0][1], 50)
ypl = np.linspace(cls_ackley.thetalimits[1][0], cls_ackley.thetalimits[1][1], 50)
Xpl, Ypl = np.meshgrid(xpl, ypl)
th = np.vstack([Xpl.ravel(), Ypl.ravel()])
setattr(cls_ackley, 'theta', th.T)

al_ackley_test = designer(data_cls=cls_ackley, 
                            method='SEQUNIFORM', 
                            args={'mini_batch': 4, 
                                  'n_init_thetas': 10,
                                  'nworkers': 5,
...
ftest = al_ackley_test._info['f']
thetatest = al_ackley_test._info['theta']

ptest = np.zeros(thetatest.shape[0])
for i in range(ftest.shape[0]):
    mean = ftest[i] 
    rnd = sps.multivariate_normal(mean=mean, cov=cls_ackley.obsvar)
    ptest[i] = rnd.pdf(cls_ackley.real_data)
...
al_ackley = designer(data_cls=cls_ackley, 
                      method='SEQCAL', 
...
save_output(al_ackley, cls_ackley.data_name, args.al_func, args.nworkers, args.minibatch, args.seed_n0)

show = True
if show:
    theta_al = al_ackley._info['theta']
    f_al     = al_ackley._info['f']
    TV       = al_ackley._info['TV']
    HD       = al_ackley._info['HD']
    AE       = al_ackley._info['AE']
    time     = al_ackley._info['time']
