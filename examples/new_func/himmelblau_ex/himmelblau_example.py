...
cls_himmelblau = himmelblau()

# # # Create a mesh for test set # # # 
xpl = np.linspace(cls_himmelblau.thetalimits[0][0], cls_himmelblau.thetalimits[0][1], 50)
ypl = np.linspace(cls_himmelblau.thetalimits[1][0], cls_himmelblau.thetalimits[1][1], 50)
Xpl, Ypl = np.meshgrid(xpl, ypl)
th = np.vstack([Xpl.ravel(), Ypl.ravel()])
setattr(cls_himmelblau, 'theta', th.T)

al_himmel_test = designer(data_cls=cls_himmelblau, 
                            method='SEQUNIFORM', 
                            args={'mini_batch': 4, 
                                  'n_init_thetas': 10,
                                  'nworkers': 5,
...
ftest = al_himmel_test._info['f']
thetatest = al_himmel_test._info['theta']
...
al_himmel = designer(data_cls=cls_himmelblau, 
                      method='SEQCAL', 
                      args={'mini_batch': args.minibatch, 
                            ...
                            'max_evals': 1000,
                            'emutype': 'PC',
                            'candsize': 1000, #args.candsize, #args.candsize,
                            'refsize': 2500})#args.refsize})

save_output(al_himmel, cls_himmelblau.data_name, args.al_func, args.nworkers, args.minibatch, args.seed_n0)

show = True
if show:
    theta_al = al_himmel._info['theta']
    f_al     = al_himmel._info['f']
    TV       = al_himmel._info['TV']
    HD       = al_himmel._info['HD']
    AE       = al_himmel._info['AE']
    time     = al_himmel._info['time']
