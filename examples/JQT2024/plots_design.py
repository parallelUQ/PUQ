...
def plot_EIVAR(xt, cls_data, ninit, xlim1=0, xlim2=1):
    ...
    plt.scatter(
        cls_data.x,
        np.repeat(cls_data.true_theta, len(cls_data.x)),
        marker="o",
        color="black",
    )
...
def plot_des_pri(xt, cls_data, ninit, nmax):
    ...
    for label, x_count, y_count in zip(cnt, unq[:, 0], unq[:, 1]):
        plt.annotate(
            label, xy=(x_count, y_count), xytext=(5, -5), textcoords="offset points"
        )
    plt.show()
...
def plot_post(theta, phat, ptest, phatvar):
    if theta.shape[1] == 1:
        ...
        plt.fill_between(
            theta.flatten(), phat - np.sqrt(phatvar), phat + np.sqrt(phatvar), alpha=0.2
        )
        plt.show()
    else:
...
def create_test(cls_data, isbias=False):
    thetamesh = np.linspace(
        cls_data.thetalimits[1][0], cls_data.thetalimits[1][1], 100
    )[:, None]
    ...
    if cls_data.nodata:
        ...
        xt_test = np.array(
            [np.concatenate([xc, th]) for th in thetamesh for xc in cls_data.x]
        )
        ftest = np.zeros((n_t, d))
        for t_id in range(n_t):
            for x_id in range(d):
                ftest[t_id, x_id] = cls_data.function(
                    cls_data.x[x_id, 0], thetamesh[t_id, 0]
                )
...
def create_test_non(cls_data, is_bias=False):
    ...
    thetamesh = np.linspace(
        cls_data.thetalimits[2][0], cls_data.thetalimits[2][1], n_t
    )[:, None]
    ...
    for j in range(n_t):
        for i in range(n_x):
            ftest[j, i] = cls_data.function(
                cls_data.x[i, 0], cls_data.x[i, 1], thetamesh[j, 0]
            )
    ...
    if is_bias:
        ...
        ptest = np.zeros(n_t)
        for j in range(n_t):
            rnd = sps.multivariate_normal(
                mean=ftest[j, :] + biastrue, cov=cls_data.obsvar
            )
...
def observe_results(result, method, rep, ninit, nmax):
    ...
    for metric in ["TV", "HD"]:
        ...
        axes.legend(
            bbox_to_anchor=(1.1, -0.2), ncol=4, fontsize=fonts, handletextpad=0.1
        )
        plt.show()
