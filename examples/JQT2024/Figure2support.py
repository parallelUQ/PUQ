...
def ceivarxfig(
    ...
    # nx_ref x (d_x + d_t)
    xt_ref = np.concatenate(
        (x_ref, np.repeat(theta_mle, nx_ref).reshape(nx_ref, dt)), axis=1
    )
    ...
    Smat3D, rVh_1_3d, pred_mean = temp_postphimat(
        emu._info, n_x, mesh_grid, f_field_rep, obsvar3D
    )
    ...
    xnew = np.array(
        [X[ids[0].flatten(), ids[1].flatten()], Y[ids[0].flatten(), ids[1].flatten()]]
    ).reshape(1, dx + dt)

    if theta.shape[0] == 18:
        axis[1, 0].contourf(X, Y, Z, cmap="Purples", alpha=1)
        axis[1, 0].hlines(
            synth_info.true_theta,
            0,
            1,
            linestyles="dotted",
            linewidth=3,
        ...
        axis[1, 0].scatter(
            xnew[0, 0], xnew[0, 1], marker="x", c="cyan", s=200, zorder=2, linewidth=3
        )
        axis[1, 0].scatter(theta[0:10, 0], theta[0:10, 1], marker="*", c="blue", s=50)
        axis[1, 0].scatter(
            theta[10:, 0], theta[10:, 1], marker="+", c="red", s=200, linewidth=3
        )
