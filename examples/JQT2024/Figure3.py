...
for d_id in range(len(cls_data.x)):
    axs[0].scatter(
        cls_data.x[d_id, 0], cls_data.real_data[0, d_id], color="black", s=50
    )
# ft = 16
ft = 16
...
dt = len(cls_data.true_theta)
cls_data.realdata(
    x=np.array([0.1, 0.1, 0.3, 0.3, 0.5, 0.5, 0.7, 0.7, 0.9, 0.9])[:, None],
    seed=s,
    isbias=True,
)
...
for t_id, t in enumerate(th_vec):
    ...
    axs[1].plot(
        x_vec, fvec[t_id, :], label=r"$\eta(x, \theta=\pi/5)$", color="red", linewidth=3
    )
    axs[1].plot(
        x_vec,
        fvec[t_id, :] + biasvec.flatten(),
