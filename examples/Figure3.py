...
if __name__ == "__main__":
    for s in np.arange(args.seedmin, args.seedmax):
        ...
        # Initial sample
        theta0u = np.linspace(cls_func.thetalimits[0][0], cls_func.thetalimits[0][1], n0)[:, None]
        ...
        for i in range(bnew):
            ...
            cL =  np.linspace(cls_func.thetalimits[0][0], cls_func.thetalimits[0][1], nL)[:, None]
            
            mu, S, cov, cvar = get_pred(cL=cL, 
                                        emu=emu, 
                                        x=x, 
                                        ttest=theta_test, 
                                        reps=reps)
            ...
            ft = 20
            fig, ax = plt.subplots()
            ax.plot(cL, vals, color='blue')
            ax.set_xlabel(r"$\theta$", fontsize=ft)
            ax.set_ylabel("IVAR", fontsize=16)
            ax.tick_params(axis="both", labelsize=ft)
            plt.scatter(cu, minacq, color="green", marker="*", s=200)
            plt.savefig('Figure3IVAR_' + str(i) + '.png', bbox_inches='tight')
            plt.show()
            ...
            fnoise = persis_info['rand_stream'].normal(loc=cmean[0], scale=np.sqrt(cnoise[0]), size=reps)
            ...
            for tid in range(0, len(theta_test)):
                ...
                rnd = sps.norm(loc=mu[tid, 0], scale=np.sqrt(0.5*obsvar + S[tid, 0, 0]))
            ...
            ax.tick_params(axis="both", labelsize=ft)
            ax.scatter(theta0u, p0, color='black', s=100)
            ax.scatter(cu, pc, color='green', marker='*', s=200)
            plt.savefig('Figure3like_' + str(i) + '.png', bbox_inches='tight')
            plt.show()
