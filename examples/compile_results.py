...
for metric in ['AE', 'MAD']:
    ...
    for exid, ex in enumerate(example_name):
        for mid, m in enumerate(method):
            out = ex + '_' + m
            avgAE, avgtime, avgTV = plotresult(path, out, ex, worker, batch, rep, m, n0=10, nf=1000)
            if metric == 'AE':
    ...
    for metric in ['AE']:
        ...
        for exid, ex in enumerate(example_name):
            for mid, m in enumerate(method):
                ...
                avgAE, avgtime, avgTV = plotresult(path, out, ex, worker, batch, rep, m, n0=10, nf=1000)
                if metric == 'AE':
                    axes.plot(np.arange(len(avgAE)), avgAE, label=labelsb[mid], color=clist[mid], linestyle=linelist[mid])
                else:
                    axes.plot(np.arange(len(avgTV)), avgTV, label=labelsb[mid], color=clist[mid], linestyle=linelist[mid])
            axes.set_yscale('log')
            axes.set_xscale('log')
        ...
        for exid, ex in enumerate(example_name):
            for bid, b in enumerate(batch_sizes):
                for mid, m in enumerate(method):
                    ...
                    avgAE, avgtime, avgTV = plotresult(path, out, ex, worker, b, rep, m, n0=128, nf=1000)
                    if metric == 'AE':
