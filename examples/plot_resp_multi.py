...
def plot_aggregated(example_name='sinfunc', is_bias=False, rep=30):
    ...
    path = '/Users/ozgesurer/Desktop/12d/12d/'
    ...
    for metric in ['TV', 'HD']:
        ...
        for mid, m in enumerate(method):
            avgPOST, sdPOST, avgPRED, sdPRED = plotresult(path, outs, example_name, worker, batch, rep, m, n0=n0, nf=nf)
            #print(avgPRED)
            if metric == 'TV':
...
plot_aggregated(example_name='12d', is_bias=False, rep=6)